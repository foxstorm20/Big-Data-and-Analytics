...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        data_line = process_line(line)

        if data_line[11] not in list_of_bikes:
            list_of_bikes[data_line[11]] = ["universal", 0, 0]
        list_of_bikes.get(data_line[11])[1] += int(data_line[2])
        list_of_bikes.get(data_line[11])[2] += 1
    # sorted_x = sorted(list_of_bikes.items(), key=operator.itemgetter(1), reverse=True)
    # print(sorted_x)
    for key in list(list_of_bikes):
        my_str = list_of_bikes.get(key)[0] + "\t(" + str(key) + ", " + str(list_of_bikes.get(key)[1]) + ", "\
                 + str(list_of_bikes.get(key)[2]) + ")\n"
        my_output_stream.write(my_str)

    #for key in sorted_x:
    #    my_str = key[1][0] + "(" + str(key[0]) + ", " + "," + str(key[1][1]) + ", "+str(key[1][2]) + ")\n"
    #    my_output_stream.write(my_str)
