...
def process_line(line):
    ...
    bike_id = int(aux_content[0][1:])
    duration_time = int(aux_content[1])
    number_trips = int(aux_content[2][:-1])



    # 3. We assign res
    res = (universal, bike_id,duration_time, number_trips)

    # 4. We return res
    return res

def write_line(my_output_stream, bikes, duration, trip):
    # 1. We create the String
    my_str = str(bikes) + "\t" + "(" + str(duration) + ", "+str(trip)+ ")" + "\n"
...
def my_reduce(my_input_stream, my_output_stream, my_reducer_input_parameters):
    ...
    for line in my_input_stream:
        ...
        (universe, bike, duration, trips) = process_line(line)

        if universe == "universal":
            if bike not in listed_station:
                listed_station[bike] = [0, 0]
            listed_station[bike][0] += duration
            listed_station[bike][1] += trips

    sorted_x = sorted(listed_station.items(), key=operator.itemgetter(1), reverse=True)
    print(sorted_x)
    for key in sorted_x[:10]:
        write_line(my_output_stream, key[0], key[1][0], key[1][1])
