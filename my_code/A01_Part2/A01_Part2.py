...
def my_main(input_folder, output_file, top_n_bikes):
    ...
    for file in list_of_files:
        ...
        for line in my_input_stream:
            ...
            if data_line[11] not in list_of_stations:
                list_of_stations[data_line[11]] = [0,0]

            list_of_stations[data_line[11]][0] += int(data_line[2])
            list_of_stations[data_line[11]][1] += 1
    sorted_x = sorted(list_of_stations.items(), key=operator.itemgetter(1), reverse=True)
    print(sorted_x[:top_n_bikes])

    for key in sorted_x[:top_n_bikes]:
        my_str = str(key[0]) + "\t("+ str(key[1][0])+", "+ str(key[1][1])+")\n"
