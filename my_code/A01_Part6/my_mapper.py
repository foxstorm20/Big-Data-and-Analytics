...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        ...
        dates_for_data = datetime.strptime(data_line[0], '%Y/%m/%d %H:%M:%S')

        if dates_for_data.date() not in list_of_stations:
            list_of_stations[dates_for_data.date()] = []
        if int(data_line[11]) == int(my_mapper_input_parameters[0]):
            list_of_stations[dates_for_data.date()].append([data_line[0], data_line[1], data_line[4], data_line[8]])

    for key, value in list_of_stations.items():
        if not value:
            continue
        my_str += "universal \t("
        for i in value[:-1]:
            my_str += i[0] + " @ " + i[1] + " @ " + i[2] + " @ " + i[3] + " @ "
        my_str += value[-1][0] + " @ " + value[-1][1] + " @ " + value[-1][2] + " @ " + value[-1][3] + ")\n"
