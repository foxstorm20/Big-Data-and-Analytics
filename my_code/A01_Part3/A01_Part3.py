...
def my_main(input_folder, output_file, bike_id):
    ...
    for file in list_of_files:
        ...
        for line in my_input_stream:
            ...
            if int(data_line[11]) == bike_id:

                if first:
                    first = False
                else:
                    if list_of_stations["By_Truck"][3] != data_line[4]:
                        list_of_movings[data_line[15]] = ["By_Truck", list_of_stations["By_Truck"][1], data_line[0], data_line[4], list_of_stations["By_Truck"][3]]
                list_of_stations["By_Truck"] = [data_line[0], data_line[1], data_line[4], data_line[8]]

    sorted_x = sorted(list_of_movings.items(), key=operator.itemgetter(1), reverse=False)

    for key in sorted_x:
        my_str = key[1][0] + "\t(" + key[1][1] + ", " + \
                 key[1][4]+", " + key[1][2] + ", " + \
                 key[1][3]+")\n"
