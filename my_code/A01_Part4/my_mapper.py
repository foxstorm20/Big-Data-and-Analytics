...
def my_map(my_input_stream, my_output_stream, my_mapper_input_parameters):
    ...
    for line in my_input_stream:
        ...
        if data_line[4] not in list_of_stations:
            list_of_stations[data_line[4]] = [0, 0]
        if data_line[8] not in list_of_stations:
            list_of_stations[data_line[8]] = [0, 0]

        list_of_stations.get(data_line[4])[0] += 1
        list_of_stations.get(data_line[8])[1] += 1

    for key in sorted(list_of_stations):
        my_str = key + "\t("+ str(list_of_stations.get(key)[0]) + ", " + str(list_of_stations.get(key)[1]) + ")\n"
